...
<template>
  <li v-if="notifications" class="nav-item dropdown">
    ...
    >
      ...
      <span
        v-if="notificationsCount.ref.value > 0"
        :class="`badge ${
          importantNotificationsCount.ref.value >= 1 ? 'text-bg-danger' : 'text-bg-warning'
        } border border-light rounded-pill`"
      >
        {{ notificationsCount.ref }}
      ...
      <ul class="list-group list-group-flush">
        <li class="list-group-item">
          <div class="d-flex align-items-center">
            <div>
              Notifications<template v-if="notificationsCount.ref.value > 0">
                &nbsp;({{ notificationsCount.ref.value }})
              </template>
            </div>
            ...
            <div class="ms-auto">
              ...
              <button
                class="btn text-body"
                :class="{ 'text-muted': notificationsCount.ref.value <= 0 }"
        ...
        <template v-if="notifications.ref.value.length > 0">
          <li
            v-for="item in getFilteredNotifications(notifications.ref.value)"
            :key="item.id"
